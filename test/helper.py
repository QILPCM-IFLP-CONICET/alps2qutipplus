...
test_cases_states["z semipolarized"] = ProductDensityOperator(
    {name: 0.5 * qutip.qeye(2) + 0.25 * qutip.sigmaz()
...
test_cases_states["x semipolarized"] = ProductDensityOperator(
    {name: 0.5 * qutip.qeye(2) + 0.25 * qutip.sigmax()
     for name in system.dimensions},
    1.0,
    system=system,
)
...
test_cases_states["first full polarized"] = ProductDensityOperator(
    {sx_A.site: 0.5 * qutip.qeye(2) + 0.5 * qutip.sigmaz()},
    1.0,
    system=system
)
...
test_cases_states[
    ...
    {sx_A.site: 0.5 * qutip.qeye(2) + 0.25 * qutip.sigmaz()},
    1.0,
    system=system
) + 0.5 * ProductDensityOperator(
    {sx_B.site: 0.5 * qutip.qeye(2) + 0.25 * qutip.sigmaz()},
    1.0,
    system=system
)


test_cases_states["gibbs_sz"] = GibbsProductDensityOperator(sz_total,
                                                            system=system)
