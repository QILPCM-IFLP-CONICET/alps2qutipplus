...
from alpsqutip.proj_evol import (  # # function used to safely and robustly map K-states to states
...
def fn_Hij_tensor_with_errors(basis, sp: Callable, generator):
    ...
    hgen = -1j * generator
    print(type(hgen))
    print([type(op2) for op2 in basis])
...
def project_operator_to_m_body(full_operator: Operator, m_max=2, sigma_0=None):
    ...
    if isinstance(full_operator, SumOperator):
        terms = tuple(
            (project_operator_to_m_body(term, m_max, sigma_0) for term in terms)
        )
        return SumOperator(terms, system).simplify()

    if isinstance(full_operator, ProductOperator):
        # reduce op1 (x) op2 (x) op3 ...
        # to <op1> Proj_{m}(op2 (x) op3) + Delta op1 (x) Proj_{m-1}(op2 (x) op3)
        # and sum the result.
        ...
        if sigma_0 is not None:
            ...
            op_first = op_first * sigma_first
        first_av = op_first.tr()
    ...
    if not isinstance(full_operator, QutipOperator):
        full_operator = full_operator.to_qutip_operator()

    # Reduce a qutip operator
    site_names = full_operator.site_names
    first_site, *rest = tuple(site_names)
    sigma_rest = sigma_0
    if sigma_0:
        sigma_first = sigma_0.partial_trace(frozenset({first_site}))
        sigma_rest = sigma_0.partial_trace(frozenset(rest))
        full_operator * sigma_first
