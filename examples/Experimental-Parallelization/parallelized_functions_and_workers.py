    ...
    for i in range(1, deep):
        ...
        with ProcessPoolExecutor(max_workers=num_workers) as executor:
            projected_terms = list(
                executor.map(_projection_worker, projection_args, chunksize=128)
            )
...
def compute_Hij_tensor_non_orth(
    basis, generator, sp, sigma_ref, m_max, Gram=None, num_workers=4
):
    """
    ...
    args = [(i, basis[i], basis[-1], generator, sp, sigma_ref, m_max) for i in range(n)]
...
def orthogonalize_basis_parallel_process(
    basis, sp: callable, tol=1e-6, max_workers=None
):
    """
...
def parallel_gram_schmidt_with_gram(basis, sp: callable, tol=1e-6, num_threads=4):
    ...
    with ThreadPoolExecutor(max_workers=num_threads) as executor:
        ...
        results = list(
            executor.map(
                lambda ij: (ij[0], ij[1], sp(basis[ij[0]], basis[ij[1]])), pairs
            )
        )
    ...
    for k, b_k in enumerate(basis):
        ...
        with ThreadPoolExecutor(max_workers=num_threads) as executor:
            projections = list(executor.map(lambda q_j: sp(q_j, q_k), orth_basis))
